  ...
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        ...
        <!-- 添加业主区域 -->
        <el-col :span="1">
          <el-button type="primary" @click="dialogFormVisible = true">添加业主</el-button>
          <el-dialog
            title="添加业主"
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        is_f: [{ required: true, message: '请选择是否是业主', trigger: 'blur' }]
      }
    };
  },
  // 页面加载调用
  ...
  methods: {
    ...
    // 添加业主
    async addHouseHold() {
      const { data: res } = await this.$http.post('addHouseHold', this.form);
      if (res.code === 200) {
