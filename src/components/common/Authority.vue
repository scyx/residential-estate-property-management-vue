  ...
  <div>
    <el-card class="f-header">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="{ path: '/home' }"><a @click="to">首页</a></el-breadcrumb-item>
    ...
    <div class="container">
          ...
          <ul class="category-list">
            <li v-for="item in authority_group_list" :key="item.authority_group_id" class="item_li">
              ...
              <el-button
                ...
                type="danger"
                size="mini"
                @click="deleteById(item.authority_group_id)"
              >删除</el-button>
            </li>
          </ul>
        </div>
      ...
      <el-card class="main">
        <div class="main-header">
          <h3>{{title}}</h3>
          <el-button type="primary" class="selectButton" size="medium" icon="el-icon-plus" @click="test">添加权限</el-button>
        </div>
        <el-divider></el-divider>
        <el-table
          :data="tableData"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '权限组名不能为空', trigger: 'blur' }
        ],
        description: [
